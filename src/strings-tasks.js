...
function countVowels(str) {
  ...
  const sum = str
    .split('')
    .filter((letter) => ARR_VOWELS.includes(letter)).length;
  return sum;
}
...
function isPalindrome(str) {
  return (
    str
      .split('')
      .map((letter) => letter.replace(/[^a-zA-Z]+/g, ''))
      .join('')
      .toLowerCase() ===
    str
      .split('')
...
function reverseWords(str) {
  ...
  const reverseArr = strSplit.map((word) => word.split('').reverse().join(''));
...
function getCardId(value) {
  ...
  return ARR_CARDS.indexOf(value);
}
